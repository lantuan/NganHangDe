...
\begin{vd}
	Một công ty sản xuất và bán hết $x$ sản phẩm ($0 < x \le 1000$), tổng số tiền công ty thu được là $f(x) = 1000x - x^2$ (nghìn đồng), chi phí sản xuất bình quân cho một sản phẩm là $g(x) = x + \dfrac{30}{x} + 680$ (nghìn đồng). Giả sử mức thuế phụ thu trên một đơn vị sản phẩm bán được là $t$ (nghìn đồng) ($0 < t < 200$). Tìm mức thuế phụ thu $t$ (nghìn đồng) trên một sản phẩm sao cho nhà nước nhận được số tiền thuế phụ thu lớn nhất và công ty cũng thu được lợi nhuận lớn nhất theo mức thuế phụ thu đó.
	\loigiai{
		Tổng chi phí để sản xuất $x$ sản phẩm là:
		$$ C(x) = x \cdot g(x) = x \left( x + \dfrac{30}{x} + 680 \right) = x^2 + 680x + 30 \text{ (nghìn đồng)}. $$
		Tổng số tiền thuế công ty phải nộp cho nhà nước khi bán $x$ sản phẩm là:
		$$ T(x) = t \cdot x \text{ (nghìn đồng)}. $$
		Lợi nhuận của công ty thu được khi bán $x$ sản phẩm là:
		$$ P(x) = f(x) - C(x) - T(x) = (1000x - x^2) - (x^2 + 680x + 30) - tx $$
		$$ P(x) = -2x^2 + (320 - t)x - 30. $$
		Để công ty thu được lợi nhuận lớn nhất, ta tìm cực đại của hàm số $P(x)$ trên khoảng $(0; 1000]$.\\
		Ta có $P'(x) = -4x + 320 - t$.\\
		$P'(x) = 0 \Leftrightarrow 4x = 320 - t \Leftrightarrow x = \dfrac{320 - t}{4}$.\\
		Vì $0 < t < 200$ nên $120 < 320 - t < 320 \Rightarrow 30 < x < 80$, hoàn toàn thỏa mãn điều kiện $x \in (0; 1000]$.\\
		Do hệ số của $x^2$ là $-2 < 0$, parabol $P(x)$ quay bề lõm xuống dưới, nên hàm số $P(x)$ đạt giá trị lớn nhất tại đỉnh $x = \dfrac{320 - t}{4}$.\\
		Với mức sản lượng $x$ này, số tiền thuế nhà nước thu được là một hàm số biến $t$:
		$$ R(t) = t \cdot x = t \cdot \dfrac{320 - t}{4} = -\dfrac{1}{4}t^2 + 80t. $$
		Bài toán tiếp tục yêu cầu tìm $t \in (0; 200)$ để hàm số $R(t)$ đạt giá trị lớn nhất.\\
		Ta có $R'(t) = -\dfrac{1}{2}t + 80$.\\
		$R'(t) = 0 \Leftrightarrow -\dfrac{1}{2}t + 80 = 0 \Leftrightarrow t = 160$ (thỏa mãn $0 < t < 200$).\\
		Hàm số $R(t)$ cũng là một hàm bậc hai có hệ số $a = -\dfrac{1}{4} < 0$ nên đạt giá trị lớn nhất tại $t = 160$.\\
		Vậy mức thuế phụ thu trên một sản phẩm cần tìm là $160$ nghìn đồng.
	}
\end{vd}
\dongcham{9}
...
\begin{vd}
	...
	\immini{Một bể nước có dạng hình nón ngược với bán kính đáy bằng 2 m và chiều cao bằng 4 m (tham khảo hình vẽ dưới đây). Nước được bơm vào bể với tốc độ không đổi là $2 \mathrm{~m}^3$/phút. Hỏi tốc độ dâng lên của mực nước (đơn vị $\mathrm{m}$/phút ) bằng bao nhiêu khi mực nước trong bể đạt độ sâu bằng 3 m (làm tròn kết quả dến hàng phần trăm)?
	}{
...
\end{tikzpicture}}
	...
	\begin{enumerate}[\iconCV]
		\item \indamm{Cách 1:} Gọi $x$ là thời gian bơm (phút). Sau $x$ phút thì
		\begin{itemize}
			\item [$\bullet$] Thể tích bể nước tính theo tốc độ bơm là $V=2x$ \quad(1);
			\item [$\bullet$] Thể tích bể nước tính theo công thức hình học là $V=\dfrac{1}{3}\pi r^2h$. Theo hình vẽ thì $\dfrac{r}{2}=\dfrac{h}{4} \Rightarrow r=\dfrac{h}{2}$. Suy ra $V=\dfrac{1}{3}\pi \left(\frac{h}{2} \right)^2 h=\dfrac{\pi}{12}h^3$ \quad(2).
		\end{itemize}
	Từ (1) và (2), ta có $\dfrac{\pi}{12}h^3=2x \Rightarrow h=\sqrt[3]{\dfrac{24x}{\pi}}$. Với $h=3$ thì $x=\dfrac{9\pi}{8}$. Suy ra tốc độ dâng cần tìm là $h'(3) \approx 0,28$.
		\item \indamm{Cách 2:} Ta có tốc độ dâng (thẳng đứng) bằng tỉ số giữa tốc độ bơm với diện tịc mặt thoáng. Với dữ kiện đề bài này, thì
		\begin{itemize}
			\item [$\bullet$] Tốc độ bơm $2 m^3$/phút.
			\item [$\bullet$] Diện tích mặt thoáng (diện tích phần mặt nước) $S=\pi r^2 = \pi \times \left( \dfrac{3}{2}\right)^2=\dfrac{9\pi}{4}$. 
			Tốc độ dâng là $2 : \dfrac{9\pi}{4} \approx 0,28$
		\end{itemize}
	\end{enumerate}	
	}
\end{vd}
...
\cham{6}
\begin{ex}
	Một vật chuyền động theo quy luật $s=-\dfrac{1}{3} t^3+6 t^2$ với $t$ (giây) là khoảng thời gian tính từ khi vật bắt đầu chuyển động và $s$ (mét) là quãng đường vật di chuyền được trong khoảng thời gian đó. Hỏi trong khoảng thời gian 9 giây, kể từ khi bắt đầu chuyển động, vận tốc lớn nhất của vật đạt được là bao nhiêu?(\textit{kết quả tính bằng m/s})
	\choice
	{54 m/s}
	{36 m/s}
	{\True  27 m/s}
...
\begin{ex}%[2D1C3-6]
	...
	\loigiai{
		...
		Số sản phẩm A bán được sau khi áp dụng giảm giá là $100+10x$ (sản phẩm).\\
		Với mỗi sản phẩm A, cửa hàng thu được ($20-x$ nghìn đồng) lợi nhuận.\\ Vậy, tổng lợi nhuận của cửa hàng là 
		$$
		f(x)=(100+10x)(20-x)=-10x^2+100x+2000 \quad (x<20). 
		$$
		$f'(x)=-20x+100$\\
		
		$f'(x)=0 \Leftrightarrow x=5$.
		Bảng biến thiên
		\begin{center}
			\begin{tikzpicture}
				\tikzset{double style/.append style={double distance=2pt}}
				\tkzTabInit[lgt=1.2, espcl=2]
				{$x$/0.6,$f'(x)$/0.6,$f(x)$/1.5}{$0$,$5$,$20$}
				\tkzTabLine{,+,0,-,}
				\tkzTabVar{-/$2\,000$,+/$2250$,-/$0$}
			\end{tikzpicture}
		\end{center}
		Tổng lợi nhuận lớn nhất là $2250$ nghìn đồng đạt được khi giảm giá $5$ nghìn đồng cho mỗi sản phẩm.	
	}
\end{ex}
\cham{8}
\begin{ex}
