...
\begin{dang}{Tính góc trong không gian Oxyz}
	\begin{itemize}
		\item [$\bullet$] Xác định véc tơ chỉ phương (vec tơ pháp tuyến);
...
\begin{vd}
	...
	{
	\begin{enumEX}[a)]{1}
		\item Ta có $\vec{n}_P=(1;1;4)$, $\vec{n}_Q=(2;0;-2)$ lần lượt là véc-tơ pháp tuyến của $(P)$ và $(Q)$.\\
		...
		\item $(P)\colon 2x - y-2z-9=0$ có $1$ véc-tơ pháp tuyến là $\overrightarrow{n_1} = (2;-1;-2)$.\\
...
\begin{vd}%[2H3B3-4]%
	...
	\loigiai{
		\begin{enumEX}[a)]{1}
			...
			\item Mặt phẳng $(P)$ có véc-tơ pháp tuyến $\overrightarrow{n}=(4;3;-1)$.\\
			Đường thẳng $d$ có véc-tơ chỉ phương $\overrightarrow{u}=(4;3;1)$.\\
...
\begin{ex}
	...
	{$\cos \varphi=-\dfrac{\sqrt{6}}{9}$}
	\loigiai
	{Đường thẳng $d_1$, $d_2$ lần lượt có véc-tơ chỉ phương là $\overrightarrow{u}_1=(-1; 2; 2)$ và $\overrightarrow{u}_2=(2; 0; -1)$.\\
...
\begin{ex}
	...
	\loigiai{
		...
		Véc-tơ chỉ phương của $d$ là $\vec{u}_d=\left[\vec{n}_P;\vec{n}_Q \right]=\left(\sin \alpha;\cos \alpha;1\right)$.\\
		Véc-tơ chỉ phương của trục $Oz$ là $\vec{k}=(0;0;1)$.\\
...
\begin{ex}%[2-TT-5- Đề thi tháng 2-2019, Toán 12 trường THPT chuyên Bắc Giang- 2019]%[Nguyễn Thế Anh-EX6]%[2H3B3-4]%
	...
	\loigiai
	{
		Ta có $\vec {u}=(-1;2;1)$ là véc-tơ chỉ phương của đường thẳng $d$ và $\vec{n}=(1;-1;0)$ là véc-tơ pháp tuyến của mặt phẳng $(P)$.\\
...
\begin{ex}
	...
	{$60^{\circ}$}
	\loigiai{
		Mặt phẳng $(P)$ có một véc-tơ pháp tuyến là $\overrightarrow{n}=(3;4;5)$.\\
		Đường thẳng $d$ có một véc-tơ chỉ phương là $\overrightarrow{u}=(-3;-4;-5)$.\\
...
\begin{ex}
	...
	{\True $\varphi=30^{\circ}$}
	\loigiai{
		Ta có véc-tơ pháp tuyến của mặt phẳng $(P)$ là $\overrightarrow{n}=(1;1;-\sqrt{2})$ và véc-tơ chỉ phương của trục $Oy$ là $\overrightarrow{u}=(0;1;0)$. Suy ra $\sin\varphi=\dfrac{|\overrightarrow{u}\cdot \overrightarrow{n}|}{|\overrightarrow{u}|\cdot|\overrightarrow{n}|}=\dfrac{|1|}{\sqrt{4}\cdot 1}=\dfrac{1}{2}\Rightarrow\varphi=30^{\circ}$.
	}
\end{ex}
\cham{3}
\begin{ex}
...
\begin{ex}%[Trần Bình Thuận - DA2]%[2H3K2-5]% Câu 7
	...
	{\True $ \dfrac{1 }{2} $}
	\loigiai{
		Véc-tơ pháp tuyến của mặt phẳng $ (P): \vec{n}_{(P)} = (1;2;-1) $.\\
		Véc-tơ pháp tuyến của mặt phẳng $ (Q): \vec{n}_{(Q)} = (1;-m;m+1) $.\\
...
\begin{vd}%[2H5V1-6]
	...
	{
		\immini{
			Xét hệ trục tọa độ $Oxyz$ sao cho các điểm có tọa độ như sau: $O(0;0;0)$, $O'(2a;0;0)$, $B(0;3a;0)$, $C(0;0;a)$.\\
			Trong không gian $Oxyz$ vừa chọn, ta có $B'(2a;3a;0)$, $C'(2a;0;a)$, $\overrightarrow{BO}'=(0;-3a;0)$, $\overrightarrow{CB}'=(2a;3a;-a)$.
			\begin{enumerate}
		...
		Suy ra $\left ((O'BC),(OBC) \right )\approx 31^{\circ}$.
		\begin{enumerate}
			\setcounter{enumi}{2}
...
\begin{vd}%[2H5V2-8]
	...
	\loigiai{
		\begin{listEX}
			...
			Và  
			\begin{eqnarray*}
				&\sin \beta=\dfrac{|\overrightarrow{NA} \cdot \overrightarrow{n}|}{|\overrightarrow{N A}| \cdot|\overrightarrow{n}|} & =\dfrac{|5 \cdot(-2)+2 \cdot(-21)+4 \cdot(-26)|}{\sqrt{5^2+2^2+4^2} \cdot \sqrt{(-2)^2+(-21)^2+(-26)^2}} \\
...
\begin{vd}
	...
	\begin{center}
		\includegraphics[width=6cm]{hinh/KhuonBanh.jpg}~
	...
	\loigiai{
		\begin{listEX}[1]
			...
			$\sin\alpha=\dfrac{\left|\overrightarrow{ST}\cdot\vec{k}\right|}{\left|\overrightarrow{ST}\right|\cdot\left|\vec{k}\right|}=\dfrac{7}{\sqrt{51}}\Rightarrow \alpha\approx 79^\circ$.
...
\begin{ex}
	\immini[thm]{
		...
		\includegraphics[width=4cm]{hinh/MayBay.jpg}
	}
	\loigiai{Gọi $\alpha$ là góc nâng của máy bay.\\
		$\sin\alpha=\dfrac{\left|\overrightarrow{v}\cdot\vec{k}\right|}{\left|\overrightarrow{v}\right|\cdot\left|\vec{k}\right|}\Rightarrow \alpha\approx 10^\circ 4'$.
	}
\end{ex}
\cham{5}
...
\begin{ex}
	...
	\loigiai{
		\immini
		{ Chọn hệ tọa độ $Oxyz$ có $A\equiv O$, $ \overrightarrow{AB}$, $\overrightarrow{AD}$, $\overrightarrow{AA'}$ lần lượt cùng hướng với các véc-tơ đơn vị $\overrightarrow{i}$, $\overrightarrow{j}$, $\overrightarrow{k}$.\\
			Lấy $a=1$, suy ra $B(1;0;0)$, $D(0;1;0)$, $A'(0;0;1)$, $C(1;1;0)$.\\
			Mặt phẳng $(BA'C)$ có véc-tơ pháp tuyến là \\ $\overrightarrow{n_1}=\left[\overrightarrow{BA'}; \overrightarrow{BC}\right]=(-1;0;-1)$.\\
			Mặt phẳng $(DA'C)$ có véc-tơ pháp tuyến là \\ $\overrightarrow{n_2}=\left[\overrightarrow{DA'}; \overrightarrow{DC}\right]=(0;1;1)$.\\
...
\begin{ex}
	...
	\loigiai{
		\immini{
			Do $OA$, $OB$, $OC$ đôi một vuông góc nên chọn hệ trục toạ độ như hình vẽ. Khi đó, ta có hệ toạ độ của các điểm $O(0;0;0)$, $A(0;0;a)$, $B(a;0;0)$, $C(0;a;0)$.\\
			Suy ra $M\left(\dfrac{a}{2};0;\dfrac{a}{2}\right)$, $N\left(0;\dfrac{a}{2};0\right)$ nên $\overrightarrow{N M}=\left(\dfrac{a}{2} ; -\dfrac{a}{2} ;\dfrac{a}{2}\right)$.\\
...
\begin{ex}
	...
	\loigiai{
		\immini{
			...
			Ta có $ \overrightarrow {BD}  = (-a;a\sqrt{3};0) = a(-1;\sqrt{3};0) $ nên đường thẳng $ BD $ có véc-tơ chỉ phương là $ \overrightarrow {u} = (-1;\sqrt{3};0) $.\\
			Ta có  $\overrightarrow {SB} = (a;0;-a)$, $ \overrightarrow {BC} = (0;a\sqrt{3};0) \Rightarrow \left[\overrightarrow {SB} ,\overrightarrow {BC} \right] = \left( {a^2 \sqrt{3}; 0; a^2\sqrt{3}} \right) = a^2 \sqrt{3}  (1;0;1)$. Nên mặt phẳng $ (SBC) $ có véc-tơ pháp tuyến là $ \overrightarrow {n} = (1;0;1) $.\\
...
\begin{ex}
	...
	\loigiai{
		...
		Ta có \begin{eqnarray*}
			\sin \left(MN;(ABCD)\right)= \dfrac{\left|\vec{u}_{MN} \cdot \vec{n}_{ABCD}\right|}{\left|\vec{u}_{MN}\right| \cdot \left|\vec{n}_{ABCD}\right|} & \Leftrightarrow & \sin 60^{\circ} =\dfrac{\dfrac{k}{2}}{\sqrt{\dfrac{5a^2}{8}+\dfrac{k^2}{4}}} \\
...
\begin{ex}
	...
	\loigiai{
		\immini{
			...
			Lại có: $\dfrac{a}{2} \vec{MN}=(0;3;-1)= \vec{w}$. \\
