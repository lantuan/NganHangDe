	...
	\begin{center}
		\begin{tikzpicture}
			...
			row 2/.style={nodes={minimum height=7mm}},
				...
				Nhóm &$[u_1;u_2)$&$[u_2;u_3)$&\dots&$[u_p;u_{p+1})$&\dots&$[u_k;u_{k+1})$\\ 
				\node[align=center]{Tần số}; &$n_1$&$n_2$&\dots&$n_p$&\dots&$n_k$\\
			};
		\end{tikzpicture}
	\end{center}
	Nếu $n_1$ và $n_k$ cùng khác $0$ thì khoảng biến thiên của mẫu số liệu ghép nhóm được tính theo công thức
		\boxmini{$R=u_{k+1}-u_1$}
	\item \indam{Ý nghĩa:} Khoảng biến thiên của mẫu số liệu ghép nhóm là giá trị xấp xỉ khoảng biến thiên của mẫu số liệu gốc và có thể dùng để đo mức độ phân tán của mẫu số liệu. Khoảng biến thiên càng lớn thì mẫu số liệu càng phân tán.
\end{enumerate}

\subsubsection{Khoảng tứ phân vị}
	\indamm{Nhắc lại công thức tính tứ phân vị thứ $r$:} 
$$Q_r=u_p+\dfrac{ \dfrac{r\cdot n}{4} -(n_1+n_2+\ldots + n_{p-1})}{n_p}(u_{p+1}-u_p)$$
	Trong đó $[u_p;u_{p+1})$ là nhóm chứa tứ phân vị thứ $r$, với $r=1, 2, 3$.
\begin{enumerate}[\iconMT] 
...
\begin{vd}
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	Tìm khoảng biến thiên cho thời gian sử dụng mạng xã hội của học sinh. Căn cứ vào đó, nhận xét thời gian sử dụng mạng xã hội của các bạn Tổ nào phân tán hơn.
	\loigiai{
...
\end{ex}
\begin{ex}
	Thời gian truy cập Internet mỗi tối của một số học sinh được cho trong bảng sau:
	%	\vspace*{-10pt}
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|}
...
\begin{dang}{Tìm khoảng tứ phân vị của mẫu số liệu ghép nhóm}
	\begin{listEX}[1]
		\item [\ding{172}] Tìm tứ phân vị $ Q_1$ và $Q_3 $ theo công thức:
		$$Q_r=u_p+\dfrac{\dfrac{r \cdot n}{4}-\left(n_1+\cdots+n_{p-1}\right)}{n_p} \cdot\left(u_{p+1}-u_p\right), $$
		trong đó $\left[u_p; u_{p+1}\right)$ là nhóm chứa tứ phân vị thứ $r$ với $r=1$, $3$; \quad $n$ là cỡ mẫu.
...
\setcounter{vd}{0}
\begin{vd}
	Cho bảng số liệu ghép nhóm về cân nặng của $ 50 $ quả xoài:
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|}
...
\begin{vd}%[2D3H1-4]
	...
	\end{center}
	\begin{enumerate}
		\item Tính khoảng biến thiên, khoảng tứ phân vị cho các mẫu số liệu ghép nhóm của học sinh lớp 12A, 12B.
...
\begin{vd}
	\immini
	{
		...
		\begin{luuy}
			Tần số tích lũy của nhóm thứ $m$ chính bằng $n_1+n_2+\cdots+n_m$. Trong đó, $n_1$, $n_2$,..., $n_m$ lần lượt là tần số của nhóm thứ $1$, thứ $2$,..., thứ $m$.
		\end{luuy}
	}
	{
		\begin{tabular}{|c|c|c|}
...
\begin{ex}%[1D1B2-2]
	Cho bảng số liệu ghép nhóm về cân nặng của các học sinh:
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|}
...
\begin{ex}%[2D3H1-3]
	...
	\begin{center}
		\begin{tikzpicture}[font=\footnotesize, line join=round, line cap=round, >=stealth,x=0.25cm,y=0.6cm]
...
\begin{ex}
	...
	\begin{center}
		...
		\end{tikzpicture}\\	
		\textit{Hình 3.3a. Diện tích nhà ở của cư dân địa bàn $A$}
		%========================
	\end{center}
	\begin{center}
		\begin{tikzpicture}[>=stealth,scale=1.5]
			...
			%========================
		\end{tikzpicture}\\
		\textit{Hình 3.3b. Diện tích nhà ở của cư dân địa bàn $B$}
	\end{center}
	\choiceTF
