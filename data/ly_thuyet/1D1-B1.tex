...
\begin{itemize}
	\item[\iconMT] \indam{Góc lượng giác và số đo của góc lượng giác:} Trong mặt phẳng, cho hai tia $Ou$, $Ov$. Xét tia $Om$ cùng nằm trong mặt phẳng này. 
	\item [] \indamm{Ghi nhớ 1:}
		\begin{itemize}
			\item 	Nếu tia $Om$ quay quanh điểm $O$, theo một chiều nhất định từ $Ou$ đến $Ov$, thì ta nói nó quét một góc lượng giác với tia đầu $Ou$, tia cuối $Ov$ và kí hiệu là $(\mathrm{O} u, \mathrm{O} v)$.
		\end{itemize}
	\hspace*{1.5cm}
	...
	\item [] \indamm{Ghi nhớ 2:}
		\begin{itemize}
			...
			\end{tikzpicture}
			\item Số đo của các góc lượng giác có cùng tia đầu $Ou$ và tia cuối $Ov$ sai khác nhau một bội nguyên của $360^{\circ}$ nên có công thức tổng quát là 
			\boxminit{$\text{sđ}(Ou, Ov)=\alpha^{\circ}+k 360^{\circ}, \text{ với } k \in \mathbb{Z}$}
		\end{itemize}
	\item[\iconMT] \indam{Hệ thức Chasles:} Với ba tia $Ou$, $Ov$, $Ow$ bất kì, ta có
	$$\text{sđ}(Ou,Ov)+\text{sđ}(Ov,Ow)=\text{sđ}(Ou, Ow)+k360^{\circ} \quad \text{với } k \in \mathbb{Z}.$$
\end{itemize}

\subsubsection{ĐƠN VỊ ĐO GÓC VÀ ĐỘ DÀI CUNG TRÒN}
\begin{itemize}
	\item[\iconMT] \indam{Đơn vị đo góc:}
	\begin{itemize}
...
\begin{itemize}
	\item[\iconMT] \indam{Đường tròn lượng giác:} 
	\immini{
		\begin{itemize}
	...
	\item[\iconMT] \indam{Các giá trị lượng giác của góc lượng giác }
	\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
		\indamm{ Ghi nhớ 1: } Giả sử $M(x_0;y_0)$ trên đường tròn lượng giác biểu diễn cho góc lượng giác có số đo $\alpha$ thì
		\begin{itemize}
...
\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
	\indamm{Ghi nhớ 2:} Ta có các kết quả sau được suy ra từ định nghĩa
	\begin{itemize}
...
\begin{itemize}
	\item[\iconMT] \indam{Công thức lượng giác cơ bản:}
	...
	\item[\iconMT] \indam{Giá trị lượng giác của các góc có liên quan đặc biệt:}
	\begin{listEX}[1]
		\item [\ding{172}] Hai góc đối nhau: $\alpha $ và $-\alpha$:
		\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
			...
			\end{tikzpicture}}
		\end{tcolorbox}
		\item [\ding{173}] Hai góc bù nhau: $\alpha $ và $\pi-\alpha $.
		\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
			\immini{	\begin{itemize}
			...
			\end{tikzpicture}}
		\end{tcolorbox}
		\item [\ding{174}] Hai góc hơn kém nhau $\pi$: $\alpha $ và $\alpha+\pi$
		\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
			\immini{	\begin{itemize}
			...
			\end{tikzpicture}}
		\end{tcolorbox}
		\item [\ding{175}] Hai góc phụ nhau: $\alpha $ và $\dfrac{\pi}{2}-\alpha$
		\begin{tcolorbox}[colframe=cyan,colback=red!1!white,boxrule=0.5mm]
			\immini{	\begin{itemize}
...
\begin{dang}{Tính giá trị của biểu thức $M$ liên quan đến các giá trị lượng giác}
	\begin{itemize}
		\item [\iconMT] \indam{Hướng 1:}
		\begin{itemize}
			\item  Từ tỉ số lượng giác đã cho, ta tính toán các giá trị lượng giác có trong biểu thức $M$.
			\item  Thay tất cả giá trị vừa tìm được vào $M$, suy ra kết quả.
		\end{itemize}
		\item [\iconMT] \indam{Hướng 2:}
		\begin{itemize}
			\item  Biến đổi biểu thức $M$ về tỉ số lượng giác đã cho.
...
\begin{baitap}%[Tex hóa SGK CD-CT,T7/22, TVN-099]%[1T1K1-3]
...
\begin{baitap}%[Tex hóa SGK CD-CT,T7/22, TVN-099]%[1T1B1-6]
