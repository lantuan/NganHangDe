\begin{dang}{Bài toán cực trị có chứa tham số}
 ...
 `\begin{itemize}
 	\item [\ding{172}] Độ dài $MN=\sqrt{(x_N-x_M)^2+(y_N-y_M)^2}$
 	\item [\ding{173}]  Khoảng cách từ $M$ đến $\Delta$: $d(M,\Delta)=\dfrac{|Ax_M+By_M+C|}{\sqrt{A^2+B^2}}$, với $\Delta \colon Ax+By+C=0$.
 	\item [\ding{174}] Tam giác $ABC$ vuông tại $A \Leftrightarrow \overrightarrow{AB} \cdot \overrightarrow{AC}=0 \Leftrightarrow \text{hoành}\cdot\text{hoành}+\text{tung}\cdot\text{tung}=0$.
 	\item [\ding{175}] Diện tích tam giác $ABC$ là  $S=\dfrac{1}{2}|a_1b_2-a_2b_1|$, với $\overrightarrow{AB}=(a_1;b_1)$, $\overrightarrow{AC}=(a_2;b_2)$.
 \end{itemize}
	\end{enumerate}
\end{dang}
