...
\begin{vd}
	...
	\loigiai{
		...
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		Từ bảng biến thiên, ta thấy $\min\limits_{[0;+\infty)}y(x)=\dfrac{5}{2}$ và $\mathop{\rm{max}}\limits_{[0;+\infty)}y(x)=5$.
	}
\end{vd}
\dongcham{7}
...
\begin{vd} %[Word to LaTeX 3.2]
	\immini{Chào đón năm mới $2025$, Thành phố trang trí đèn led cho biểu tượng hình chữ $V$ được ghép từ các thanh $AB=4$m, $AC=5$m sao cho tam giác $ABC$ vuông tại $B$(tham khảo hình vẽ)
		Để tăng hiệu ứng các kỹ sư đã thiết kế một chuỗi led chạy từ $B$ xuống $A$ với vận tốc $4$m/phút và một chuỗi led chạy từ $A$ đến $C$ với vận tốc $10$m/phút. Sau khi đóng nguồn điện thì cả hai chuỗi led đồng thời xuất phát. Hỏi sau bao giây từ thời điểm đóng nguồn điện thì khoảng cách giữa hai điểm sáng đầu tiên của hai chuỗi led là nhỏ nhất?
	}{
		\begin{tikzpicture}[scale=0.8, thick,>=stealth]
			...
			;
			\draw (B)--(A)--(C);
			\foreach \x/\g in {A/-90,B/180,C/0}\draw[fill=black] (\x) circle (.04) +(\g:.3)node{$\x$};
	\end{tikzpicture}}
	\loigiai{
		Gọi $x$ (giây, $x>0$) là khoảng thời gian sau khi đóng nguồn điện hai điểm sáng gần nhau nhất.\\
		Đổi $4$m/phút$=\dfrac{1}{15}$m/s; $10$m/phút$=\dfrac{1}{6}$m/s.\\
		Giả sử sau $x$(giây), điểm sáng từ $B$ đi được quãng đường $BM=\dfrac{1}{15}x$ và quãng đường $AN=\dfrac{1}{6}x$.\\
		Áp dụng định lý cosin cho tam giác $AMN$, ta có:\\
		$MN^2=AM^2+AN^2-2AN\cdot AM\cdot \cos \widehat{MAN}$\\
		$=\left(4-\dfrac{x}{15}\right)^2+\left(\dfrac{x}{6}\right)^2-2\left(4-\dfrac{x}{15}\right)\cdot \dfrac{x}{6}\cdot \dfrac{4}{5}$\\
		$=\dfrac{{\left(60-x\right)}^2}{225}+\dfrac{x^2}{36}-\dfrac{8}{30}\dfrac{\left(60-x\right)x}{15}$\\
		$=\dfrac{3600-120x+x^2}{225}+\dfrac{x^2}{36}-\dfrac{8\cdot \left(-x^2+60x\right)}{450}$\\
		$=\dfrac{1}{20}x^2-\dfrac{8}{5}x+16$.\\
		Ta thấy $MN_{\min}\Leftrightarrow MN^2_{\min}$
		Do đó, đặt $f(x)=\dfrac{1}{20}x^2-\dfrac{8}{5}x+16, x>0$.\\
		Có $f'(x)=\dfrac{1}{10}x-\dfrac{8}{5}$
		$f'(x)=0\Leftrightarrow x=16$.
		
		Lập bảng biến thiên, suy ra $\underset{\left(0;+\infty\right)}{\min}f(x)=f(16)$.
...
\begin{ex}
	...
	\loigiai{Gọi một cạnh góc vuông là $x$ ($0<x<5$) thì cạnh góc vuông còn lại là $\sqrt{25-x^2}$.\\ Như vậy, diện tích tam giác là $S=\dfrac{x\cdot\sqrt{25-x^2}}{2}=\dfrac{1}{2}\sqrt{25x^2-x^4}$.
		...
		Vậy tam giác vuông có cạnh huyền bằng $5 \mathrm{~cm}$ có thể có diện tích lớn nhất là $S_{\max}\dfrac{1}{2}\sqrt{\dfrac{625}{4}}=\dfrac{25}{4}$.}
\end{ex}
\cham{7}
\begin{ex}
	\immini[thm]{
...
\Opensolutionfile{ans}[ans/2D1-B2-d2-2]
...
\begin{ex}
	...
	\loigiai{
		\begin{enumerate}
			...
			\item Xét hàm số $V(t)=300(t^2-t^3)+4 \text{ với } 0\le t\le 0{,}5.$ Đạo hàm $V'(t)=300(2t-3t^2)$.
			\item Xét hàm  hàm $V'(t)=300(2t-3t^2)$, có $V''(t)=300(2-6t)=0\Leftrightarrow t=\dfrac{1}{3}$\\
			Bảng biến thiên của hàm $V'(t)$ trân $[0;0.5]$
			\begin{center}
				\begin{tikzpicture}
					...
					{$0$,$\frac{1}{3}$,$0.5$}
					\tkzTabLine{,+,$0$,-,}
					\tkzTabVar{-/, +/$V\left(\frac{1}{3} \right)$,-/}
				\end{tikzpicture}
			\end{center}
			Suy ra tốc độ tăng thể tích là lớn nhất khi $t=\dfrac{1}{3}$ giờ, tương ứng khi 20 phút.
		\end{enumerate}
	}
\end{ex}
\cham{9}
...
\begin{ex}%[2D1K3-6]
	Một người thợ muốn làm một chiếc thùng hình hộp chữ nhật có đáy là hình vuông và không có nắp, biết thể tích của khối hộp là $ V=2{,}16 $ m$ ^3 $. Giá nguyên liệu để làm bốn mặt bên là $ 36000 $ đồng/m$ ^2 $ và giá nguyên liệu để làm đáy là $ 90000 $ đồng/m$ ^2 $. Gọi độ dài cạnh đáy là $ x $ (m) và chiều cao là $ h $ (m). Xét tính đúng-sai của các khẳng định sau:
	\choiceTF
	{Thể tích khối hộp được tính bởi công thức $V=x \cdot h$}
	{Mối liên hệ giữa $x$ và $h$ là $h=\dfrac{2{,}16}{x}$}
	{\True Để chi phí làm chiếc thùng đó thấp nhất thì độ dài cạnh đáy là $ 1{,}2 $ m}
	{Số tiền $345\,600$  đồng là chi phí thấp nhất có thể làm chiếc thùng}
	\loigiai{
		\begin{center}
		...
		\begin{itemchoice}
			...
			\begin{itemize}
				...
				\begin{center}
					...
					\end{tikzpicture}
				\end{center}
				Để chi phí làm chiếc thùng nhỏ nhất thì cạnh đáy là $x= 1{,}2 $ m.
			\end{itemize}
			\itemch Khi $x= 1{,}2 $ thì $f(x)=90x^{2}+\dfrac{7776}{25x}=388\,800$  đồng.
		\end{itemchoice}
	}
\end{ex}
\cham{12}
