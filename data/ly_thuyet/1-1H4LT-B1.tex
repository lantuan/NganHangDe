...
\begin{enumerate}[\iconMT]
	...
	\begin{tikzpicture}[scale=0.6, line join=round, line cap=round]
		...
		\tkzLabelPoints[right,font=\footnotesize](D)
		\node[below] at (0.6,-2.4) {Hình chóp tứ giác đáy hình bình hành};
	\end{tikzpicture}
	\begin{tikzpicture}[scale=0.6, line join=round, line cap=round]
...
\begin{enumerate}[\iconMT]
	...
	\begin{tcolorbox}[colframe=cyan,colback=cyan!3!white,boxrule=0.5mm]
		...
		\begin{itemize}
			\item [\ding{172}] Khi $d$ nằm trong $(\alpha)$, ta kí hiệu $d \subset (\alpha)$ hoặc $(\alpha) \supset d$ .\quad (không được viết  $d \in (\alpha)$ nhé!!!)
		...
		\end{itemize}
	\end{tcolorbox}
	\item \indam{Tính chất 5:} 
	\immini{ Nếu hai mặt phẳng phân biệt có điểm chung thì các điểm chung của hai mặt phẳng là một đường thẳng đi qua điểm chung đó.
	\begin{tcolorbox}[colframe=cyan,colback=cyan!3!white,boxrule=0.5mm]
