...
\begin{dang}{Xác định tâm $I$, bán kính $r$ của mặt cầu cho trước}
	\begin{itemize}
		...
		\begin{listEX}[1]
			\item [\ding{172}] Tâm $I\left(a,b,c\right)$ (đổi dấu hệ số của $x$, $y$, $z$ và chia đôi);
			\item [\ding{173}]  Bán kính $R=\sqrt{a^2+b^2+c^2-d}$ .
		\end{listEX}
	\end{itemize}
\end{dang}
...
\begin{vd}
	Hãy xác định tâm và bán kính (nếu là phương trình mặt cầu).
	\begin{enumEX}[a)]{2}
	...
	\loigiai{
		\begin{enumEX}[a)]{1}
			\item Mặt cầu $(S)$ có tâm $I(2;0;-1)$ và bán kính $R=\sqrt{4}=2$.
			\item Mặt cầu $(S)$ có tâm $I(1;2;-3)$ và bán kính $R=\sqrt{1^2+2^2+(-3)^2+2}=4$
			\item Ta có $a=1, b=-2, c=-\dfrac{3}{2}$ và $d=8$. Suy ra $a^2+b^2+c^2-d=-\dfrac{3}{4}<0$. Vậy phương trình đã cho không là phương trình mặt cầu.
			\item Ta có 
			\begin{eqnarray*}
...
\loigiai{
	\begin{enumEX}[a)]{1}
		\item Giả sử phương trình mặt cầu có dạng: $(S): x^2+y^2+z^2-2 a x-2 b y-2 c z+d=0$ với $a^2+b^2+c^2-d>0$.\\
		Ta có $A(1 ; 1 ; 0), B(1 ; 0 ; 1), C(0 ; 1 ; 1), D(1 ; 2 ; 3) \in(S)$.\\
		...
		$\Leftrightarrow\left\{\begin{array}{l}-2 a-2 b+d=-2 \\ -2 c-2 c+d=-2 \\ -2 b-2 c+d=-2 \\ -2 a-4 b-6 c+d=-14\end{array} \Leftrightarrow\left\{\begin{array}{l}a=\dfrac{3}{2} \\ b=\dfrac{3}{2} \\ c=\dfrac{3}{2} \\ d=4\end{array}\right.\right.$
...
\begin{vd}%GV:Phan Phú Quý
	...
	\includegraphics[width=7cm]{hinh/2P5-B4-PhuQuy-H2}
	%\includegraphics[width=7cm]{hinh/5372.pdf}\\
...
\begin{vd}
	...
	\begin{center}
		\includegraphics[height=4cm]{hinh/2P5-B4-TheUt-H3}
	\end{center}
	\loigiai{
		\immini{Xét quả bóng tiếp xúc với các bức tường và chọn hệ trục $ Oxyz $ như hình vẽ bên.\\
			...
			Giả sử $ M(x;y;z) $ nằm trên mặt cầu (bề mặt của quả bóng) sao cho $ d\left(M,(Oxy) \right)=21  $, $ d\left(M,(Oxz) \right)=18  $, $ d\left(M,(Oyz) \right)=17  $. Khi đó $ z=21, y=18, x=17 $. Khi đó ta có phương trình
			\begin{eqnarray*}
...
\begin{vd}
	...
	\loigiai{
		\immini
		{
			...
			$$h=\mathrm{d}(I;(P))=\dfrac{|4-0+2-3|}{3}=1<R.$$
			Do đó mặt phẳng $(P)$ cắt mặt cầu $(S)$.\\
