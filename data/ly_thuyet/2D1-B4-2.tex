\begin{dang}{Đồ thị hàm số phân thức hữu tỉ $y=\dfrac{ax+b}{cx+d}$}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Lập bảng biến thiên; xác định chiều biến thiên của hàm số.
		\end{itemize}
		\item [\iconCH] \indamm{Bước 3.} Vẽ hai đường tiệm cận; cho thêm điểm và vẽ đồ thị của hàm số dựa vào bảng biến thiên.
	\end{itemize}
\end{dang}
...
\begin{ex}
	...
	{
		\begin{tikzpicture}[smooth,samples=300,scale=0.48,>=stealth]
...
\begin{ex}
	Cho hàm số $y=\dfrac{5 x - 1}{- 2 x - 2}$. Xét tính đúng-sai của các khẳng định sau.
	\choiceTF
	{ \True Hàm số có $y'=\dfrac{-12}{\left(- 2 x - 2\right)^{2}}$ }
	{ \True Hàm số nghịch biến trên các khoảng $\left(-\infty;-1\right)$ và $\left(-1;+\infty\right)$ }
	{ Hàm số có bảng biến thiên là\\
	\hspace*{3cm}\begin{tikzpicture}[scale=1]
		\tikzset{double style/.append style = {draw=\tkzTabDefaultWritingColor,double=\tkzTabDefaultBackgroundColor,double distance=2pt}}
			\tkzTabInit[nocadre=false,lgt=1.5,espcl=4]
			{$x$ /0.7,$y'$ /0.7,$y$ /2}
			{$-\infty$,$-1$,$+\infty$}
			\tkzTabLine{,+, d ,+,}
			\tkzTabVar{-/$\frac{5}{2}$ / , +D-/ $+\infty$ /$-\infty$  , +/ $\frac{5}{2}$/}
		\end{tikzpicture}
	}
	{ \True Hàm số có đồ thị là\\
		\hspace*{3cm}\begin{tikzpicture}[line join=round, line cap=round,>=stealth,thick,scale=0.6]
			\tikzset{every node/.style={scale=0.9}}
			\draw[gray!20](-6,-7)grid(4,3);
			\draw[->] (-6,0)--(4,0) node[below left] {$x$};
			\draw[->] (0,-7)--(0,3) node[below left] {$y$};
			...
			\begin{scope}
				...
				\draw[samples=200,domain=-6:-1.1,smooth,variable=\x]
				plot (\x,{(5*(\x)+-1)/(-2*(\x)+-2)});
				\draw[samples=200,domain=-0.9:4,smooth,variable=\x]
				plot (\x,{(5*(\x)+-1)/(-2*(\x)+-2)});
				\draw[dashed,samples=200,domain=-6:4,smooth,variable=\x]
				plot (\x,{(-2.5});
