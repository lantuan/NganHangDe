...
\begin{minipage}[b]{10cm}
	...
	\begin{minipage}[b]{6.5cm}
		\begin{khung4}{GHI NHỚ}
			...
			\ding{175} Tọa độ tâm đối xứng của đồ thị có hoành độ tâm đối xứng là nghiệm phương trình $y''=0 \Leftrightarrow x=-\dfrac{b}{3a}$.
		\end{khung4}
		\vspace{1 cm}
	\end{minipage}
	\vspace{-1 cm}
...
\begin{dang}{Đồ thị hàm số bậc ba $y=ax^3+bx^2+cx+d$}
	\begin{itemize}
		\item[\iconCH] \indamm{Bước 1.} Tìm tập xác định của hàm số.
		\item [\iconCH] \indamm{Bước 2.} Khảo sát sự biến thiên của hàm số
		\begin{itemize}
			\item Tính đạo hàm $y'$. Giải $y'=0$, tìm nghiệm.
			\item Tìm các giới hạn tại vô cực, giới hạn vô cực.
			\item Lập bảng biến thiên; Kết luận chiều biến thiên và cực trị của hàm số.
		\end{itemize}
...
\begin{ex}
	Cho hàm số $y=\dfrac{x^{3}}{3} + \dfrac{3 x^{2}}{2} + 2 x - 1$. Xét tính đúng-sai của các khẳng định sau. 
	\choiceTF
	{ Hàm số có đạo hàm là $y'=x^{2} + 3 x + 4$ }
	{ \True $y'>0$ khi $x\in (-\infty;-2)\cup (-1;+\infty)$ }
	{ \True Hàm số có bảng biến thiên là\\
		\hspace*{3cm}	\begin{tikzpicture}
			\tkzTabInit[nocadre=false, lgt=1, espcl=2] 
			{$x$ /0.7,$y'$ /0.7,$y$ /2}
			{$-\infty$,$-2$,$-1$,$+\infty$}
			\tkzTabLine{,+,0,-,0,+,} 
			\tkzTabVar{-/$-\infty$ ,+/ $- \frac{5}{3}$, -/ $- \frac{11}{6}$ /, +/$+\infty$ /} 
		\end{tikzpicture}
	}
	{ Hàm số có đồ thị là\\
		\hspace*{4cm}\begin{tikzpicture}[line join=round, line cap=round,>=stealth,thick,scale=0.6]
			...
			\begin{scope}
				...
				plot (\x,{-0.3333333333333333*(\x)^3+-1.5*(\x)^2+-2*(\x)+1});\end{scope}
		\end{tikzpicture}
	}
	\loigiai{ 
		...
		$y'=0\Leftrightarrow x=-2,x=-1$.
		
		$y'>0$ khi $x\in (-\infty;-2)\cup (-1;+\infty)$.
		
		$y'<0$ khi $x\in (-2;-1)$.
		
		c) Khẳng định đã cho là khẳng định đúng.
		
		$x=-2\Rightarrow y=- \frac{5}{3}, x=-1\Rightarrow y=- \frac{11}{6}$.
