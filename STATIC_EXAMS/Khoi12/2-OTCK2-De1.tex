...
%\begin{ex}%[2D4H2-5]
%Cho hàm số $f\left( x \right)$ liên tục trên $\mathbb{R}$ và thỏa mãn $\displaystyle\int\limits_{-5}^1 f\left( x \right)\mathrm{d}x=9$. Tích phân $\displaystyle\int\limits_0^2 \left[ f\left( 1-3x \right)+9 \right]\mathrm{d}x$ bằng
%\choice
%{$27$}
%{\True $21$}
%{$15$}
%{$75$}
%\loigiai{
	%Đặt $t=1-3x \Rightarrow \mathrm{d}t=-3\mathrm{d}x$. Với $x=0\to t=1$ và $x=2\to t=-5$.\\
	%Ta có
	%\allowdisplaybreaks
	%\begin{eqnarray*}
	%\displaystyle\int\limits_0^2 \left[ f\left( 1-3x \right)+9 \right]\mathrm{d}x &=& \displaystyle\int\limits_0^2 f\left( 1-3x \right)\mathrm{d}x+\displaystyle\int\limits_0^2 9\mathrm{d}x\\
	%&=& \displaystyle\int\limits_1^{-5} \left[ f\left( t \right) \right]\dfrac{\mathrm{d}t}{-3}+9x\Big|_0^2\\
	%&=& \dfrac{1}{3}\displaystyle\int\limits_{-5}^1 f\left( x \right) \mathrm{d}x+18\\
	%&=& \dfrac{1}{3}\cdot 9+18=21.
	%\end{eqnarray*}
	%}
%\end{ex}
...
\Closesolutionfile{ans}
%\indapan{6}{ans/ans-2-HK2-De1-TLN}   